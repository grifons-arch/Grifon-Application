...
const normalizeCountryIso = (countryIso?: string): string =>
  countryIso?.trim().toUpperCase() === "SE" ? "SE" : "GR";

const resolveCountryLabel = (request: RegisterRequest, countryIso: string): string => {
...
const readWholesaleResponseMetadata = (response: any) => {
...
const buildWholesaleSubmissionResponse = (
  response: any,
  ...
  const message = metadata.wholesaleApplicationRegistered
    ...
    : buildWholesaleFailureMessage(
        metadata.wholesaleApplicationSkippedReason,
        metadata.wholesaleApplicationTable,
        metadata.wholesaleApplicationError
      );
  ...
  const message = wholesaleRequested
    ? (
        wholesaleMetadata.wholesaleApplicationRegistered
          ? buildWholesaleSuccessMessage(
              wholesaleMetadata.wholesaleApplicationTable,
              wholesaleMetadata.wholesaleApplicationMode
            )
          : buildWholesaleFailureMessage(
              wholesaleMetadata.wholesaleApplicationSkippedReason,
              wholesaleMetadata.wholesaleApplicationTable,
              wholesaleMetadata.wholesaleApplicationError
            )
      )
...
export const submitWholesaleApplication = async (request: RegisterRequest): Promise<any> => {
  const email = request.email.trim().toLowerCase();
  const countryIso = normalizeCountryIso(request.countryIso);
  ...
  return buildWholesaleSubmissionResponse(
    response,
