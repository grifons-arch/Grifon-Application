...
class FakeCatalogRepository : CatalogRepository {
    ...
    override fun getProductsByCategory(
        ...
        val base = FakeCatalogData.shopProducts[shopId].orEmpty()
            .filter { product -> product.title.contains(categoryId, ignoreCase = true) || categoryId.isBlank() }
    ...
    override fun searchProducts(
        ...
        val base = FakeCatalogData.shopProducts[shopId].orEmpty()
            .filter { product -> product.title.contains(query, ignoreCase = true) || query.isBlank() }
        return flowOf(applyFiltersAndSort(base, filters, sortOption))
    }
    ...
    private fun applyFiltersAndSort(
        ...
        filters.attributes.forEach { (key, values) ->
            if (values.isNotEmpty()) {
                filtered = filtered.filter { product -> values.contains(product.attributesMap[key]) }
            }
        }
...
class FakeUserRepository : UserRepository {
    ...
    override suspend fun login(email: String, pass: String): Boolean {
        Log.d("LoginDebug", "Fake Login attempt for: $email")
        // Προσομοίωση επιτυχίας για κάθε είσοδο
        loggedIn.value = true
        Log.d("LoginDebug", "Login State set to: ${loggedIn.value}")
        return true
    }
